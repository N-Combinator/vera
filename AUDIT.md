...
| S3 | Med | `api.py:67-73` | **No auth + wildcard CORS** (`allow_origins=["*"]`, `allow_credentials=True`) on `/scan`, `/fix`, `/describe`. Fine for localhost-only; dangerous if bound to `0.0.0.0` (which the README's Docker/uvicorn commands do). **Fix:** default-bind localhost; add an optional API token; tighten CORS. |
